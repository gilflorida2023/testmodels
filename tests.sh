rm *.csv *.json *.log 2>/dev/null
export repetitions=1
export INCLUDELIST=qwen,gemma,llava
...
export IGNORELIST=ibm,gemma3:1b
...
#python3 modelping.py "How many words in the sentence 'The rat runs fast.' begin with  'r'?" -s numeric -t 120 -exp 2  -r "$repetitions"  -e "$IGNORELIST" -i "$INCLUDELIST"
#
#
#
...
python3 modelping.py "What is the acceleration due to gravity on Earth in meters per second squared? Answer with a number with precision to hundredths." -s numeric -t 120 -sc 2 -exp 9.81 -i "$INCLUDELIST"
